...
tele_test_pred <- knn(train = tele_train, test = tele_test,
                      cl = tele_train_labels, k=182)

tele_test_pred_KNN <- ifelse(prediction < 0.5, 0, 1)
print(tele_test_pred_KNN)
...
CrossTable(x = tele_test$yyes, y = yyes_pred_ANN, prop.chisq=FALSE)
...
yyes_pred_log <- ifelse(prediction < 0.5, 0, 1)
...
```{r}
all <- yyes_pred_log + yyes_pred_ANN + tele_test_pred_KNN

all <- ifelse(prediction < 2, 0, 1)

all
```


## Alternative Logisitic Regression
```{r}
...
```
```{r}
firstprediction <- predict(firstmodel, tele_test_log)
first_yyes_pred <- ifelse(firstprediction < 0.5, 0, 1)

CrossTable(x = tele_test_log$yyes, y = first_yyes_pred, prop.chisq=FALSE)

confusionMatrix(as.factor(first_yyes_pred), as.factor(tele_test_log$yyes), positive = "1")
```
