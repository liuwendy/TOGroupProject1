...
ANN <- ANN_yyes_pred
LOG <- LOG_yyes_pred
all <- data.frame(KNN, ANN, LOG)
all$all_prediction <- KNN_yyes_pred + ANN_yyes_pred + LOG_yyes_pred
all$all_prediction <- ifelse(all$all_prediction >= 1, 1, 0)

CrossTable(x = tele_test_log$yyes, y = all$all_prediction, prop.chisq=FALSE)
confusionMatrix(as.factor(all$all_prediction), as.factor(tele_test_log$yyes), positive = "1")
```
