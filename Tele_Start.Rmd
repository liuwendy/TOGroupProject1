...
test_set1 <- sample(1:nrow(tele_cluster1), 1000)
...
ANN_yyes_pred_1 <- ifelse(prediction < 0.5, 0, 1)
...
test_set <- sample(1:nrow(tele_cluster1), 1000) 
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
# Create a train set and test set
...
all1$all_prediction <- ifelse(all$all_prediction >= 1, 1, 0)
...
```{r}
# Selects 10000 random rows for test data
test_set2 <- sample(1:nrow(tele_cluster2), 950)
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
...
```{r}
#Set a seed for random number generator for consistent output
test_set <- sample(1:nrow(tele_cluster2), 950) 
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
# Create a train set and test set
#First the predictors
...
```{r}
# Selects 10000 random rows for test data
test_set3 <- sample(1:nrow(tele_cluster3), 950)
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
...
```{r}
# Selects 10000 random rows for test data
test_set5 <- sample(1:nrow(tele_cluster5), 350)
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
...
```{r}
#Set a seed for random number generator for consistent output
test_set <- sample(1:nrow(tele_cluster5), 350) 
# Depending on R-version and computer, different rows may be selected. 
# If that happens, results are different. 
# Create a train set and test set
#First the predictors
...
#table(tele_predictors$cluster == 4)

# tele_predictors$cluster <- ifelse(tele_predictors$cluster != 4, NA, tele_predictors$cluster)
tele_predictors1 <- tele_predictors[tele_clusters$cluster == 1,]
tele_predictors2 <- tele_predictors[tele_clusters$cluster == 2,]
tele_predictors3 <- tele_predictors[tele_clusters$cluster == 3,]
tele_predictors4 <- tele_predictors[tele_clusters$cluster == 4,]
tele_predictors5 <- tele_predictors[tele_clusters$cluster == 5,]

clusterpred1 <- predict(firstmodel, tele_predictors1)
clusterpred2 <- predict(firstmodel, tele_predictors2)
clusterpred3 <- predict(firstmodel, tele_predictors3)
clusterpred4 <- predict(firstmodel, tele_predictors4)
clusterpred5 <- predict(firstmodel, tele_predictors5)

LOG_yyes_pred1 <- ifelse(clusterpred1 < 0.5, 0, 1)
# CrossTable(x = tele_test_log$yyes, y = LOG_yyes_pred1, prop.chisq=FALSE)
# confusionMatrix(as.factor(LOG_yyes_pred1), as.factor(tele_test_log$yyes), positive = "1")

LOG_yyes_pred2 <- ifelse(clusterpred2 < 0.5, 0, 1)
# CrossTable(x = tele_test_log$yyes, y = LOG_yyes_pred2, prop.chisq=FALSE)
# confusionMatrix(as.factor(LOG_yyes_pred2), as.factor(tele_test_log$yyes), positive = "1")

LOG_yyes_pred3 <- ifelse(clusterpred3 < 0.5, 0, 1)
# CrossTable(x = tele_test_log$yyes, y = LOG_yyes_pred4, prop.chisq=FALSE)
# confusionMatrix(as.factor(LOG_yyes_pred4), as.factor(tele_test_log$yyes), positive = "1")

LOG_yyes_pred4 <- ifelse(clusterpred4 < 0.5, 0, 1)
LOG_yyes_pred5 <- ifelse(clusterpred5 < 0.5, 0, 1)
